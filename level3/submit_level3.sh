...
"$PYTHON_BIN" "$SCRIPT_DIR/prepare_level3.py" "${prepare_args[@]}"
...
array_job="$(sbatch "${array_args[@]}" "$SCRIPT_DIR/run_level3_array.sh" "$OUTDIR")"
...
aggregate_job="$(sbatch "${aggregate_args[@]}" "$SCRIPT_DIR/aggregate_level3.sh" "$OUTDIR")"
echo "level3_array_job=$array_job"
echo "level3_aggregate_job=$aggregate_job"
